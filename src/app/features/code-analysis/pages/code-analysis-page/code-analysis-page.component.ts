  ...
  template: `
    <div class="container page-layout">
      ...
      <div class="error-banner glass-card" *ngIf="facade.error()">
        ...
        <button type="button" class="error-dismiss" (click)="facade.reset()">Descartar</button>
      </div>

      <!-- Hero Introduction -->
      <div class="page-intro" *ngIf="!facade.hasReport()">
        <div class="title-with-badge">
          <h1 class="intro-title">Ingeniería Inversa & Síntesis de Arquitectura</h1>
        </div>
        <p class="intro-desc">
          Analiza cualquier repositorio público <strong>GitHub</strong> o paquete <strong>.ZIP</strong> para extraer métricas factuales de arquitectura, estereotipos de componentes e inferencia técnica asistida por <strong>IA</strong>.
        </p>
      </div>
      ...
      <div class="loader-wrapper" *ngIf="facade.loading()">
        ...
        <div class="pipeline-legend">
          <span class="legend-title">Pipeline Determinístico de 7 Etapas en Ejecución:</span>
      ...
      <div class="results-dashboard" *ngIf="facade.hasReport() && facade.result() as report">
        ...
        <div class="dashboard-header glass-card">
          <div class="header-info">
            ...
            <div class="repo-meta">
              <span class="meta-badge" [class.git]="report.sourceType === 'GITHUB_REPO'">
                {{ report.sourceType === 'GITHUB_REPO' ? 'GitHub Repository' : 'ZIP File Archive' }}
              </span>
              <span class="meta-time">Concluido: {{ formatTimestamp(report.timestamp) }}</span>
            </div>
          </div>
          <button type="button" class="btn-new-analysis" (click)="onResetAll()">+ Evaluar otro repositorio</button>
        </div>
        ...
        <app-architecture-evidence [evidence]="report.architectureEvidence"></app-architecture-evidence>

        <!-- 4. Catálogo de Componentes Identificados -->
        <app-components-catalog [analysis]="report.componentAnalysis"></app-components-catalog>
        ...
        <app-prompt-inspector [context]="report.analysisContext"></app-prompt-inspector>
      </div>
    </div>
  `,
  styles: [`

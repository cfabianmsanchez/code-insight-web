...
export class CodeAnalysisFacade {
  ...
  readonly hasReport = computed(
    () => this._status() === "SUCCESS" && this._result() !== null,
  );
  readonly activeModel = computed(
    () => this._systemConfig()?.ollamaModel || "",
  );
  readonly availableModels = computed(
    () => this._systemConfig()?.availableModels ?? [],
  );
  ...
  selectModel(modelName: string): void {
    ...
    this.apiService.updateActiveModel(modelName.trim()).subscribe({
      ...
      error: (err) =>
        console.error(
          "Error al actualizar el modelo de IA en el backend:",
          err,
        ),
    });
  }
  ...
  analyzeGithubRepo(repoUrl: string, projectKey?: string): void {
    if (!repoUrl || repoUrl.trim().length === 0) {
      this._error.set(
        "Por favor ingresa una URL válida de repositorio GitHub.",
      );
      return;
    }
    ...
    this.apiService.analyzeGithubRepo(dto).subscribe({
      ...
      error: (err) => {
        const errorMsg =
          ...
          err?.error?.message ||
          err?.message ||
          "Error al conectar con la API de análisis.";
    ...
    this.apiService.analyzeZipFile(file, calculatedKey).subscribe({
      ...
      error: (err) => {
        const errorMsg =
          ...
          err?.error?.message ||
          err?.message ||
          "Error al procesar el archivo ZIP subido.";
        this._error.set(errorMsg);
        this._status.set("ERROR");
      },
    });

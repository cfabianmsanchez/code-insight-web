...
@Component({
    ...
    <div class="synthesis-card glass-card" *ngIf="synthesis">
      ...
      <div class="synthesis-formatted-body">
        <ng-container *ngFor="let sec of formattedSections">
          ...
          <p *ngIf="sec.type === 'p'" class="synth-p" [innerHTML]="parseBold(sec.content)"></p>
          <ul *ngIf="sec.type === 'ul'" class="synth-ul">
            <li *ngFor="let item of sec.items" [innerHTML]="parseBold(item)"></li>
          </ul>
        </ng-container>
      </div>
    </div>
  ...
  styles: [`
    ...
    ::ng-deep .synth-p strong, ::ng-deep .synth-ul strong {
      color: #f8fafc;
      font-weight: 700;
    }
    ::ng-deep .synth-p code, ::ng-deep .synth-ul code {

...
@Component({
    ...
    <div class="tech-stack-card glass-card" *ngIf="stack">
      ...
      <div class="stack-grid">
        <div class="stack-item">
          ...
          <span class="item-value badge language">{{ stack.mainLanguage || 'No detectado' }}</span>
        </div>

        <div class="stack-item">
          <span class="item-label">Framework Principal</span>
          <span class="item-value badge framework">{{ stack.mainFramework || 'No detectado' }}</span>
        </div>

        <div class="stack-item">
          <span class="item-label">Herramienta de Compilación</span>
          <span class="item-value badge build">{{ stack.buildTool || 'No detectada' }}</span>
        </div>

        <div class="stack-item">
          <span class="item-label">Bases de Datos</span>
          <div class="tags-list">
            <span *ngFor="let db of stack.databasesDetected" class="tag db">{{ db }}</span>
            <span *ngIf="!stack.databasesDetected?.length" class="tag empty">Ninguna observada</span>
          </div>
        </div>
      </div>
      ...
      <div class="libraries-section" *ngIf="stack.keyLibraries?.length">
        ...
        <div class="tags-list">
          <span *ngFor="let lib of stack.keyLibraries" class="tag lib">{{ lib }}</span>
        </div>
      </div>

      <!-- Conteo por extensión -->
      <div class="extensions-section" *ngIf="extensionCounts && extensionCountKeys.length">
  ...
  styles: [`
    ...
    .badge.language { background: rgba(59, 130, 246, 0.15); color: #3b82f6; }
    .badge.framework { background: rgba(16, 185, 129, 0.15); color: #10b981; }
    .badge.build { background: rgba(245, 158, 11, 0.15); color: #f59e0b; }

    .libraries-section, .extensions-section {
      display: flex;
      flex-direction: column;
      gap: 10px;
      padding-top: 12px;
    ...
    .tag.lib { background: var(--bg-hover); color: var(--text-main); border: 1px solid var(--border-color); }
    ...
    .ext-name { font-weight: 700; color: var(--text-main); }
    .ext-count { font-weight: 600; color: var(--text-muted); }
  `]
})

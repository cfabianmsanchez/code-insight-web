...
@Component({
    ...
    <div class="arch-evidence-card glass-card" *ngIf="evidence">
      <div class="card-header">
        <div class="header-title-row">
          ...
          <span class="kind-badge" *ngIf="evidence.projectKind">{{ evidence.projectKind }}</span>
          <span class="framework-badge" *ngIf="evidence.frontendFramework && evidence.frontendFramework !== 'NONE'">{{ evidence.frontendFramework }}</span>
        </div>
        <p class="card-subtitle">Evidencias determinísticas de paquetes, puertos, adaptadores y prácticas de ingeniería</p>
      </div>
      ...
      <div class="relations-grid">
        ...
        <div class="relation-box inbound">
          <div class="box-header">
            ...
            <span class="box-title">Puerto de Entrada (Caso de Uso) ➔ Servicio (Implementación)</span>
          </div>
          ...
          <div class="relations-list" *ngIf="inboundKeys.length">
            <div *ngFor="let port of inboundKeys" class="relation-item">
              ...
              <span class="impl-name">{{ evidence.inboundPortImplementations[port] }}</span>
            </div>
          </div>
        ...
        <div class="relation-box outbound">
          <div class="box-header">
            ...
            <span class="box-title">Adaptador (Infraestructura) ➔ Puerto de Salida</span>
          </div>
          ...
          <div class="relations-list" *ngIf="outboundKeys.length">
            <div *ngFor="let adapter of outboundKeys" class="relation-item">
              ...
              <span class="port-name">{{ evidence.outboundAdapterImplementations[adapter] }}</span>
            </div>
          </div>
      ...
      <div class="engineering-box" *ngIf="evidence.engineeringEvidence">
        ...
        <div class="engineering-grid">
          <div class="eng-item">
            ...
            <span class="eng-val" [class.positive]="evidence.engineeringEvidence.testFilesDetected > 0">
              {{ evidence.engineeringEvidence.testFilesDetected }} archivos
            </span>
          </div>

          <div class="eng-item" *ngIf="evidence.engineeringEvidence.testDirectories?.length">
            <span class="eng-label">Directorios de Test</span>
            <span class="eng-val">{{ evidence.engineeringEvidence.testDirectories.join(', ') }}</span>
          </div>

          <div class="eng-item" *ngIf="evidence.engineeringEvidence.testScript">
            <span class="eng-label">Script de Test (package.json)</span>
            <span class="eng-val code"><code>{{ evidence.engineeringEvidence.testScript }}</code></span>
          </div>

          <div class="eng-item" *ngIf="evidence.engineeringEvidence.springConfigurationDetected">
            <span class="eng-label">Inyección de Dependencias (&#64;Configuration)</span>
            <span class="eng-val positive">DETECTADA</span>
          </div>

          <div class="eng-item" *ngIf="evidence.engineeringEvidence.beanDefinitions > 0">
            <span class="eng-label">Definiciones &#64;Bean</span>
            <span class="eng-val">{{ evidence.engineeringEvidence.beanDefinitions }} beans</span>
          </div>

          <div class="eng-item" *ngIf="evidence.engineeringEvidence.constructorInjectionDetected">
      ...
      <div class="package-distribution" *ngIf="packageKeys.length">
        <span class="box-subtitle">Distribución de Componentes por Capa / Paquete</span>
        <div class="packages-list">
          <div *ngFor="let pkg of packageKeys" class="package-tag">
            <span class="pkg-name">{{ pkg }}</span>
            <span class="pkg-count">{{ evidence.packageComponentDistribution[pkg] }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .inbound-badge { background: rgba(59, 130, 246, 0.2); color: #3b82f6; border: 1px solid rgba(59, 130, 246, 0.3); }
    .outbound-badge { background: rgba(168, 85, 247, 0.2); color: #a855f7; border: 1px solid rgba(168, 85, 247, 0.3); }
    ...
    .impl-name { font-weight: 700; color: var(--text-main); }
    .relation-arrow { font-size: 0.75rem; color: var(--text-muted); }
    ...
    .engineering-box, .package-distribution {
      display: flex;
      flex-direction: column;
      gap: 14px;
      padding: 20px 24px;
    ...
    .eng-label { font-size: 0.75rem; font-weight: 600; color: var(--text-muted); }
    .eng-val { font-size: 0.92rem; font-weight: 700; color: var(--text-main); }
    .eng-val.positive { color: var(--success); }
    ...
    .pkg-count { font-weight: 700; color: var(--primary); }
  `]
})

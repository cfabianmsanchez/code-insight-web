...
import { ComponentAnalysisResult, DetectedComponent } from '../../models/analysis-response.model';
...
@Component({
  ...
  template: `
    <div class="components-card glass-card" *ngIf="analysis">
      <div class="card-header">
        <h3 class="card-title">🧩 Catálogo de Componentes Identificados ({{ analysis.totalComponents }})</h3>
        <p class="card-subtitle">Clases y componentes detectados por estereotipo de arquitectura</p>
      </div>
        ...
        <table class="components-table">
          ...
          <tbody>
            <tr *ngFor="let comp of filteredComponents">
              <td class="comp-name"><code>{{ comp.name }}</code></td>
              <td>
                <span class="stereotype-tag" [ngClass]="getStereotypeClass(comp.type)">
                  {{ comp.type }}
                </span>
              </td>
              <td class="comp-path" [title]="comp.relativePath"><code>{{ comp.relativePath }}</code></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  styles: [`
    ...
    .comp-name { font-weight: 700; color: var(--text-main); }
    .comp-path {
      font-size: 0.85rem;
      color: var(--text-muted);
      max-width: 320px;
    ...
    .st-controller { background: rgba(59, 130, 246, 0.2); color: #3b82f6; }
    .st-service { background: rgba(16, 185, 129, 0.2); color: #10b981; }
    .st-repository { background: rgba(245, 158, 11, 0.2); color: #f59e0b; }
    .st-component { background: rgba(139, 92, 246, 0.2); color: #8b5cf6; }
    .st-config { background: rgba(236, 72, 153, 0.2); color: #ec4899; }
    .st-default { background: var(--bg-hover); color: var(--text-muted); }
...
export class ComponentsCatalogComponent {
  ...
  get filteredComponents(): DetectedComponent[] {
    ...
    return this.analysis.components.filter(c => c.type === this.selectedStereotype);
  }
  ...
  getStereotypeClass(type: string): string {
    switch (type.toUpperCase()) {
      case 'CONTROLLER':
      case 'RESTCONTROLLER': return 'st-controller';
      case 'SERVICE': return 'st-service';
      case 'REPOSITORY': return 'st-repository';
      case 'COMPONENT': return 'st-component';
      case 'CONFIGURATION': return 'st-config';
